...
def _family(key: str, *, hours_ago: int, verified: bool) -> dict[str, object]:
...
def test_new_verified_24h_means_recent_market_event_not_recent_crawl():
    recent_verified = _family("Recent", hours_ago=4, verified=True)
    old_verified = _family("Old", hours_ago=96, verified=True)
    fresh_lead = _family("Lead", hours_ago=2, verified=False)
